...
func TestPublicRouteCatalogueMatchesEmbeddedLegacyIndex(t *testing.T) {
	...
	for path, methods := range document.Paths {
		for method, operation := range methods {
			humaRoutes = append(humaRoutes,
				strings.ToUpper(method)+" "+
					dynamic.ReplaceAllString(path, "{}"))
			...
			if _, ok := schema.Properties["$schema"]; !ok {
				t.Errorf("%s %s response schema has no $schema property",
					method, path)
			}
			if len(schema.Properties) == 1 {
				t.Errorf("%s %s response schema is an undocumented placeholder",
					method, path)
			}
		}
	}
	sort.Strings(legacy)
	sort.Strings(humaRoutes)
	if strings.Join(legacy, "\n") != strings.Join(humaRoutes, "\n") {
		t.Fatalf("public route catalogue differs\nlegacy:\n%s\nhuma:\n%s",
			strings.Join(legacy, "\n"), strings.Join(humaRoutes, "\n"))
	}
}
...
func TestPublicIndexHonorsForwardedOrigin(t *testing.T) {
	handler := Public(Options{DB: stubDatabase{}})
...
func TestPublicHealthPreservesLegacyContract(t *testing.T) {
	handler := Public(Options{
		Version: "test-version",
		Commit:  "test-commit",
		DB:      stubDatabase{},
	...
	if strings.Contains(body, `"version"`) || strings.Contains(body, `"commit"`) {
		t.Fatalf("public health leaked replacement-only fields: %s", body)
	}
...
func TestPrivateHealthIdentifiesBuildAndLinksSchema(t *testing.T) {
	handler := Private(Options{Version: "test-version", Commit: "test-commit"})
	...
	if got := health.Header().Get("Link"); !strings.Contains(got, "</api/schemas/Health.json>") {
		t.Fatalf("health Link = %q, want private schema path", got)
	}
	...
	handler.ServeHTTP(
		schema,
		httptest.NewRequest(http.MethodGet, "http://example.com/api/schemas/Health.json", nil),
	)
