...
export default defineNuxtConfig({
  ...
  sitemap: {
    ...
    sitemaps: {
      ...
      kills: { sources: ["/_sitemap-data/kills"] },
      characters: { sources: ["/_sitemap-data/characters"] },
      coalitions: { sources: ["/_sitemap-data/coalitions"] },
      corporations: { sources: ["/_sitemap-data/corporations"] },
      alliances: { sources: ["/_sitemap-data/alliances"] },
      systems: { sources: ["/_sitemap-data/systems"] },
      regions: { sources: ["/_sitemap-data/regions"] },
      ships: { sources: ["/_sitemap-data/ships"] },
      items: { sources: ["/_sitemap-data/items"] },
      wars: { sources: ["/_sitemap-data/wars"] },
      battles: { sources: ["/_sitemap-data/battles"] },
    },
  },
  ...
  // Bun's stock Nitro entrypoint only binds TCP. Our tiny entrypoint keeps the
  // Bun preset and its export conditions, but binds NITRO_UNIX_SOCKET so Caddy
  // never needs a second public or loopback TCP listener.
  //
  // Production only, and that scoping is load-bearing. `nuxt dev` builds the
  // renderer through the nitro-dev preset, whose own entry listens on a socket
  // and reports the address back to the parent with process.send. A top-level
  // `entry` overrides that preset too, so the dev worker bound its own TCP port
  // and never completed the handshake — the dev server then answered every
  // request with "Dev server is unavailable."
  $production: {
    nitro: {
  ...
  nitro: {
    ...
    routeRules: {
      ...
      "/": {
        sitemap: { changefreq: "always", priority: 1.0 },
        headers: {
          "Cache-Control": "public, s-maxage=30, stale-while-revalidate=30",
        },
      },
      "/kills/**": {
        sitemap: { changefreq: "always", priority: 0.8 },
        headers: {
          "Cache-Control": "public, s-maxage=30, stale-while-revalidate=30",
        },
      },
      ...
      "/comments": {
        sitemap: { changefreq: "hourly", priority: 0.4 },
        headers: {
          "Cache-Control": "public, s-maxage=30, stale-while-revalidate=30",
        },
      },
  ...
  vite: {
    ...
    server: {
      ...
      // Under `make dev` the browser reaches this server through Caddy on a
      // different port, over the local HTTPS certificate. The hot module
      // replacement client builds its own WebSocket URL, and would otherwise
      // aim at Vite's own port over plain ws, where nothing is listening — so
      // the socket fails and edits stop reaching the page.
      //
      // Only applied when Caddy is in front. A bare `nuxt dev` keeps Vite's
      // default, which is already correct when the browser talks to it
      // directly.
      ...(devProxyPort
        ? { ws: { protocol: "wss", clientPort: devProxyPort } }
        : {}),
    },
    optimizeDeps: {
    ...
    resolve: {
      alias: {
        ...
        "@protobufjs/inquire": fileURLToPath(
          new URL("./shims/protobufjs-inquire.cjs", import.meta.url),
        ),
      },
    },
    build: {
