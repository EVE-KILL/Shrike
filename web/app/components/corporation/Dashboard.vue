...
const { data: statsData, pending: statsPending } = await useApiFetch<any>(`/api/corporation/${props.corporationId}/stats`, {
    params: statsParams,
    watch: [activePeriod],
...
const { data: intelData, pending: intelPending, refresh: loadIntel } = useApiFetch<any>(`/api/corporation/${props.corporationId}/intel`, {
    server: false,
    immediate: false,
    getCachedData: cachedPayload,
})
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="space-y-4">
            ...
            <div v-if="!intelPending && (allies.length > 0 || enemies.length > 0)" class="grid grid-cols-2 gap-3">
                <TopBox v-if="allies.length > 0" title="Allies" data-type="corporations" :entries="allies" count-color="text-green-400/70" hide-footer />
                <TopBox v-if="enemies.length > 0" title="Enemies" data-type="corporations" :entries="enemies" count-color="text-red-400/70" hide-footer />
            </div>
