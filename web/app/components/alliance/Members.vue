...
// Fetch corps for filter buttons
const { data: corpsData } = await useApiFetch<any>(`/api/alliance/${props.allianceId}/corporations`, {
    params: { sort: 'members', dir: 'desc' },
    getCachedData: cachedPayload,
})
const corps = computed(() => corpsData.value?.corporations || [])

// Fetch members
const memberParams = computed(() => {
    const p: Record<string, any> = { page: page.value, limit, sort: sortBy.value }
    if (corpFilter.value) p.corporation_id = corpFilter.value
    if (activityFilter.value) p.activity = activityFilter.value
    return p
})
const { data, pending } = await useApiFetch<any>(`/api/alliance/${props.allianceId}/members`, {
    params: memberParams,
    watch: [page, corpFilter, activityFilter, sortBy],
    getCachedData: cachedPayload,
})
...
const visiblePages = computed(() => {
    ...
    pages.push(1)
    if (p > 3) pages.push('...')
    for (let i = Math.max(2, p - 1); i <= Math.min(max - 1, p + 1); i++) pages.push(i)
...
const activeCorp = computed(() => {
    if (!corpFilter.value) return null
    return corps.value.find((c: any) => c.corporation_id === corpFilter.value) || null
})

const corpDropdownOpen = ref(false)

const selectCorp = (corpId: number) => {
    corpFilter.value = corpId
    page.value = 1
    corpDropdownOpen.value = false
}
    ...
    <div>
        ...
        <div v-else>
            ...
            <div class="flex flex-wrap items-center gap-3 mb-4">
                <!-- Corp filter dropdown -->
                <Dropdown v-if="corps.length > 0" v-model="corpDropdownOpen" align="left" searchable search-placeholder="Filter corporations...">
                    <template #trigger>
                        <button class="flex items-center gap-2 px-3 py-1.5 text-fine rounded font-medium transition-colors border whitespace-nowrap"
                            :class="corpFilter ? 'bg-blue-500/20 text-blue-400 border-blue-500/30' : 'text-gray-400 border-white/[0.08] hover:border-white/[0.15]'">
                            <img v-if="activeCorp" :src="`/images/corporations/${activeCorp.corporation_id}/logo?size=32`"
                                class="w-4 h-4 rounded flex-shrink-0">
                            <Icon v-else name="lucide:building-2" class="w-3.5 h-3.5 flex-shrink-0" />
                            <span class="truncate max-w-[200px]">{{ activeCorp ? `${activeCorp.name} [${activeCorp.ticker}]` : 'All Corps' }}</span>
                            <Icon name="lucide:chevron-down" class="w-3 h-3 ml-1 opacity-50 flex-shrink-0" />
                        </button>
                    </template>
                    <template #default="{ close, search }">
                        <button v-if="!search"
                            class="flex items-center gap-2.5 w-full px-3 py-2 rounded-md text-xs transition-colors"
                            :class="!corpFilter ? 'text-blue-400 bg-blue-500/[0.08]' : 'text-gray-400 hover:text-blue-400 hover:bg-blue-500/[0.08]'"
                            @click="selectCorp(0)">
                            <Icon name="lucide:building-2" class="w-4 h-4 opacity-50" />
                            All Corporations
                        </button>
                        <button v-for="c in corps.filter((c: any) => !search || c.name.toLowerCase().includes(search.toLowerCase()) || c.ticker.toLowerCase().includes(search.toLowerCase()))" :key="c.corporation_id"
                            class="flex items-center gap-2.5 w-full px-3 py-2 rounded-md text-xs transition-colors whitespace-nowrap"
                            :class="corpFilter === c.corporation_id ? 'text-blue-400 bg-blue-500/[0.08]' : 'text-gray-400 hover:text-blue-400 hover:bg-blue-500/[0.08]'"
                            @click="selectCorp(c.corporation_id)">
                            <img :src="`/images/corporations/${c.corporation_id}/logo?size=32`"
                                class="w-4 h-4 rounded" loading="lazy">
                            {{ c.name }}
                            <span class="text-gray-600 ml-auto">[{{ c.ticker }}]</span>
                        </button>
                    </template>
                </Dropdown>
                <!-- Activity filter -->
            ...
            <div class="flex items-center justify-between mb-3">
                <div class="text-xs text-gray-500">
                    ...
                    <span v-if="activeCorp" class="text-gray-600">in {{ activeCorp.name }}</span>
                ...
                <div v-if="totalPages > 1" class="flex items-center gap-1">
                    <button @click="goToPage(page - 1)" :disabled="page <= 1"
                        ...
                        :class="page <= 1 ? 'text-gray-700' : 'text-gray-400 hover:text-blue-400 hover:bg-blue-500/[0.06]'">
                    ...
                    <button @click="goToPage(page + 1)" :disabled="page >= totalPages"
                        class="px-2 py-1 text-xs rounded transition-colors"
                        :class="page >= totalPages ? 'text-gray-700' : 'text-gray-400 hover:text-blue-400 hover:bg-blue-500/[0.06]'">
            ...
            <!-- Members grid -->
            <div v-if="members.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-1.5" :class="{ 'opacity-50': pending }">
