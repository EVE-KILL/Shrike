...
<template>
    <div>
        ...
        <div v-else class="glass-panel" :class="{ 'opacity-60': pending }">
            <div class="hidden md:grid grid-cols-[1fr_auto_1fr_120px_100px] gap-3 px-4 py-2 text-fine font-bold uppercase tracking-wider text-gray-500 border-b border-white/[0.08]">
                <div>Aggressor</div>
                <div></div>
                <div>Defender</div>
                <div>Started</div>
            ...
            <NuxtLink v-for="w in wars" :key="w.war_id" :to="`/war/${w.war_id}`"
                class="hidden md:grid grid-cols-[1fr_auto_1fr_120px_100px] gap-3 px-4 py-3 border-b border-white/[0.04] hover:bg-blue-500/[0.04] transition-colors items-center">
                <div class="flex items-center gap-2.5 min-w-0">
                    <EveImage :src="entityImage(w.aggressor)" :size="32" :alt="w.aggressor.name" class="w-8 h-8 rounded flex-shrink-0" />
                    <div class="min-w-0">
                        ...
                        <div class="text-fine text-gray-500">
                            ...
                            <span class="tabular-nums">{{ w.aggressor.ships_killed }} ships</span>
                        </div>
                    </div>
                </div>
                <div class="flex items-center justify-center text-xs text-gray-600 font-bold px-2">VS</div>
                <div class="flex items-center gap-2.5 min-w-0">
                    <EveImage :src="entityImage(w.defender)" :size="32" :alt="w.defender.name" class="w-8 h-8 rounded flex-shrink-0" />
                    <div class="min-w-0">
                        ...
                        <div class="text-fine text-gray-500">
                            ...
                            <span class="text-gray-600 mx-1">·</span>
                            <span class="tabular-nums">{{ w.defender.ships_killed }} ships</span>
                        </div>
                    </div>
                </div>
                <div class="text-xs text-gray-400">{{ timeAgo(w.started) }}</div>
            ...
            <NuxtLink v-for="w in wars" :key="`m-${w.war_id}`" :to="`/war/${w.war_id}`"
                class="md:hidden block px-3 py-3 border-b border-white/[0.04] hover:bg-blue-500/[0.04] transition-colors">
                <div class="flex items-center gap-2">
                    <div class="flex items-center gap-1.5 flex-1 min-w-0">
                        <EveImage :src="entityImage(w.aggressor)" :size="32" :alt="w.aggressor.name" class="w-7 h-7 rounded flex-shrink-0" />
                        <div class="min-w-0">
                            <div class="text-fine text-gray-200 truncate">{{ w.aggressor.name }}</div>
                            <div class="text-fine text-isk/70 tabular-nums">{{ formatIsk(w.aggressor.isk_destroyed) }}</div>
                        </div>
                    </div>
                    <div class="text-fine text-gray-600 font-bold px-1">VS</div>
                    <div class="flex items-center gap-1.5 flex-1 min-w-0 justify-end text-right">
                        <div class="min-w-0">
                            <div class="text-fine text-gray-200 truncate">{{ w.defender.name }}</div>
                            <div class="text-fine text-isk/70 tabular-nums">{{ formatIsk(w.defender.isk_destroyed) }}</div>
                        </div>
                        <EveImage :src="entityImage(w.defender)" :size="32" :alt="w.defender.name" class="w-7 h-7 rounded flex-shrink-0" />
                    </div>
                </div>
                <div class="flex items-center justify-between mt-1.5">
                    <div class="text-fine text-gray-500">{{ timeAgo(w.started) }}</div>
