...
const entityType = ref(
  validEntityTypes.has(initialEntityType as any)
    ? initialEntityType
    : "character",
);
const rankingWindow = ref(
  validWindows.has(initialWindow as any) ? initialWindow : "all_time",
);
...
const selectedEntityLabel = computed(
  () =>
    entityTypes.find((item) => item[0] === entityType.value)?.[1] ?? "Entities",
);
const selectedWindowLabel = computed(
  () =>
    windows.find((item) => item[0] === rankingWindow.value)?.[1] ?? "All Time",
);
...
useSeoMeta({
  ...
  description:
    "Explore live hourly, weekly, 90-day, and all-time EVE-KILL combat rankings.",
});
</script>
...
<template>
  <main class="max-w-6xl mx-auto px-4 py-6 space-y-5">
    <PageHeader
      ...
      icon="lucide:trophy"
      description="Compare the most active characters, corporations, alliances, ships, systems, and regions across eight time periods."
    />
    ...
    <div class="flex flex-wrap gap-2 justify-between">
      ...
      <div class="flex gap-1.5">
        <button
          ...
          class="px-3 py-1.5 rounded border text-xs transition-colors"
          :class="
            rankingWindow === item[0]
              ? 'border-emerald-500/40 bg-emerald-500/10 text-emerald-300'
              : 'border-white/10 text-gray-400 hover:text-white'
          "
          @click="rankingWindow = item[0]"
        >
          {{ item[1] }}
        </button>
      </div>
    ...
    <section v-if="!pending && podium.length" class="grid gap-3 sm:grid-cols-3">
      <NuxtLink
        ...
        :class="
          entry.overall_rank === 1
            ? 'border-yellow-400/25 bg-yellow-400/[0.05] sm:-translate-y-1'
      ...
      >
        <div class="flex items-center gap-3">
          <div class="relative">
            <EveImage
              :src="entityImage(entry)"
              :size="64"
              :alt="entry.name"
              class="h-14 w-14 rounded-lg bg-gray-900 object-cover ring-1 ring-white/10"
            />
            <span
              class="absolute -bottom-1.5 -right-1.5 flex h-6 min-w-6 items-center justify-center rounded-full border border-black/80 px-1 text-xs font-bold"
              :class="
                entry.overall_rank === 1
                  ? 'bg-yellow-400 text-black'
                  : 'bg-gray-700 text-gray-200'
              "
              >#{{ entry.overall_rank }}</span
            >
          </div>
          <div class="min-w-0">
            <div
              class="truncate text-sm font-semibold text-white group-hover:text-blue-300"
            >
              {{ entry.name }}
            </div>
            <div class="mt-1 text-lg font-bold tabular-nums text-blue-300">
              {{ entry.eve_kill_rating.toLocaleString("en-US") }}
            </div>
            <div class="text-fine uppercase tracking-wider text-gray-600">
              EVE-KILL rating
            </div>
          </div>
        </div>
    ...
    <div class="rounded-xl border border-white/10 bg-black/20 overflow-hidden">
      <div
        class="flex items-center justify-between border-b border-white/[0.08] px-4 py-3"
      >
        <div>
        ...
        <span class="text-fine uppercase tracking-wider text-gray-600"
          >Top 50</span
        >
      </div>
      <div
        class="hidden sm:grid grid-cols-[56px_1fr_100px_120px_100px] gap-3 px-4 py-2 border-b border-white/10 text-fine uppercase tracking-wider text-gray-600"
      >
        <span>Rank</span><span>Entity</span
        ><span class="text-right">Rating</span>
        <span class="text-right">Combat Points</span
        ><span class="text-right">Achievements</span>
      </div>
      <div v-if="pending" class="p-8 text-center text-sm text-gray-500">
        Loading rankings…
      </div>
      <NuxtLink
      ...
      >
        ...
        <span class="flex min-w-0 items-center gap-2.5"
          ><EveImage
            ...
            :size="32"
            :alt="entry.name"
            class="h-8 w-8 shrink-0 rounded bg-gray-900 object-cover"
          /><span class="truncate text-sm text-gray-200">{{
            entry.name
          }}</span></span
        >
        <span class="text-right font-bold tabular-nums text-blue-300">{{
          entry.eve_kill_rating.toLocaleString("en-US")
        }}</span>
        <span
          class="hidden text-right text-xs tabular-nums text-gray-400 sm:block"
          >{{ entry.combat_points.toLocaleString("en-US") }}</span
        >
        <span
          class="hidden text-right text-xs tabular-nums text-emerald-400 sm:block"
          >{{ entry.achievement_points.toLocaleString("en-US") }}</span
        >
      </NuxtLink>
      <div
      ...
      >
        {{
          rankingWindow === "hourly"
            ? "No scored combat activity in the last hour."
