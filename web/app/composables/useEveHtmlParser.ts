...
export const useEveHtmlParser = () => {
    ...
    const renderEveHref = (href: string): string => {
        ...
        if (href.startsWith(INVENTORY_INFO_PREFIX)) {
            ...
            const typeId = Number.parseInt(targetType[0] ?? "", 10);
     ...
     */
    const _processNode = (node: Node): void => {
        ...
        for (const childNode of childNodes) _processNode(childNode);
    };
